...
Normalization happens at comparison time in `apply_filters` and never rewrites rule source — vendor files stay committed as received. Reinterpreted values are reported in the build log — deliberately not in the manifest, where one entry per rule per date field buries everything else — and drops are recorded in the manifest under `dropped_unparsable_dates`; together they are how a recurring vendor format earns a pinned entry in `input_formats`. `parse_iso_date` remains ISO-only for policy files: leniency applies to input we don't control, never to config we author.
