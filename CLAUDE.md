...
**Relative age cutoffs.** A `meta_date` selector's `older_than: 5y` and `newer_than: 30d` (`Nd`/`Nm`/`Ny`, calendar arithmetic) are the same strict bounds as `before` and `after`, resolved against a reference date instead of written as ones; a relative bound and its absolute counterpart may both be given, and the tighter wins (earlier for the upper, later for the lower). Both relative bounds together are a rolling window. The reference date resolves `--as-of` > `as_of:` in `filters/filter_policy.yaml` > today, in `apply_filters.resolve_as_of` — the one place that chain is spelled out — and is resolved **once per run** so a build straddling midnight cannot filter two ways. A rule with no value in the named field is never aged out. This is the one deliberate qualification of NFR-6: determinism reads "identical inputs *and identical `as_of`*", which the build closes by recording the resolved `as_of` in `build_manifest.json` so `--as-of` replays it exactly. Nothing reads the reference date unless a relative bound is in the policy.
...
- **Rule output order must be deterministic** (NFR-6) — identical inputs must produce identical output on every run. A relative filter bound (`meta_date.older_than`) adds the build's `as_of` to "inputs"; it is recorded in the manifest and replayable with `--as-of`.
...
Normalization happens at comparison time in `apply_filters` and never rewrites rule source — vendor files stay committed as received. A reinterpreted value is not reported anywhere — the parse result is identical either way, and one entry per rule per date field buries everything else. Drops reach the build manifest through `filtered_rules` (null `filter_id`, `unparsable date:` reason) by virtue of that same seeding — there is no separate manifest key — which is how a recurring vendor format earns a pinned entry in `input_formats`. `parse_iso_date` remains ISO-only for policy files: leniency applies to input we don't control, never to config we author.
