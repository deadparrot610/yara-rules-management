...
| **FR-6** | The build SHALL block if any override references a vendor rule identifier that does not exist in the current vendor file (stale override detection), requiring a reviewer keep/discard decision recorded in `rules/overrides/stale_override_decisions.yaml` before the pipeline can proceed. |
