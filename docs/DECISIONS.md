...
| D-8 | **Filter tie-break: exclude wins** | 2026-06-02 | When two same-specificity filters conflict, the exclude action wins. Fail-safe toward a smaller, more deliberate deployment. Fixed in `scripts/apply_filters.py`, not configurable: the alternatives considered (`last_match_wins`, `error`) were dropped because the first makes output depend on filter list order and the second blocks the pipeline on a conflict this rule already resolves. |
