...
> **Project status: initial development.** The design is documented (see [Design documents](#design-documents)) and the repository is being built out in phases per [IMPLEMENTATION_PLAN.md](IMPLEMENTATION_PLAN.md). Commands and paths below describe the intended interface; not all scripts exist yet. This README is the contributor entry point and the build's north star.
...
**Three rule classes.** *Vendor* rules arrive as a `.yara` file from a third party and are committed as received. *Custom* rules are written in-house. *Overrides* are in-house rules that replace specific vendor rules.
...
├── vendor/vendor_rules.yara     # vendor rules, committed as received
...
python build/build_ruleset.py      # build dist/merged_rules.yara (+ .yarc) and the manifest
pytest tests/                      # run the test suite against the built ruleset
```

Build outputs land in `dist/`: `merged_rules.yara` (source), `merged_rules.yarc` (compiled, when configured), and `build_manifest.json` (rule counts by source, what was overridden, what was filtered out and by which filter, and input hashes).
...
**Update the vendor file.** Replace `vendor/vendor_rules.yara` and open a merge request so the diff and the stale-override check run before merge. A vendor update that drops a rule you were overriding will surface as a stale override.
...
| Document | Purpose |
|---|---|
| [PRD.md](PRD.md) | Requirements (functional FR-1…FR-24, non-functional NFR-1…7), scope, acceptance criteria. |
| [ARCHITECTURE.md](ARCHITECTURE.md) | Technical design: override model, filter policy model, build sequence, CI design, tooling. |
| [DECISIONS.md](DECISIONS.md) | Open decisions (D-1…D-11) with proposed defaults, plus the decision log. |
| [IMPLEMENTATION_PLAN.md](IMPLEMENTATION_PLAN.md) | Phased build order with a definition of done per phase. |
...
- A few decisions are still open and have working defaults — notably the rule consumer (D-1, sets the CI image's module support), the filter tie-break (D-8), and the filter default mode (D-9). See [DECISIONS.md](DECISIONS.md).
